...
(defn- replace-scaffold-targets
  [structure scaffold-target->dashboard-id]
  (walk/postwalk
    (fn [node]
      (if (and (vector? node) (= "scaffold-target-id" (first node)))
        (if-let [dashboard-id (get scaffold-target->dashboard-id node)]
          dashboard-id
          (throw (ex-info (i18n/tru "Unknown scaffold-target {0}" (pr-str node))
                          {:status-code 400})))
        node))
    structure))
  ...
  (let [tables (hydrate (db/select Table :id [:in table-ids]) :fields)
    ...
    {:app {:collection {:name app-name :color "#FFA500"}
           ...
           :nav_items (for [table-id table-ids]
                        {:type "dashboard"
                         :target-id ["scaffold-target-id" "page" table-id "list"]})}
  ...
  (db/transaction
    (let [{:keys [app pages cards] :as scaffold} (generate-scaffold app-name table-ids)
      ...
      (doseq [{:keys [ordered_cards scaffold-target]} pages
              ...
              dashcard ordered_cards]
        (dashboard/add-dashcard! dashboard-id (:card_id dashcard) dashcard))
      (hydrate-details (db/select-one App :id app-id)))))
